...
/*   Updated: 2025/07/13 20:01:33 by njoudieh42       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_cd(t_token *tk, t_env *env, char **ft_env)
{
	...
	if (!current_pwd)
	{
		char *old_pwd_env = get_value_from_env("PWD", env);
	...
	{
		oldpwd_env = get_value_from_env("OLDPWD", env);
		...
		if (chdir(oldpwd_env) == 0)
		{
			...
			{
				update_env_value(&env, "OLDPWD=", old_pwd);
				g_minishell.env->exit_status = ft_err_msg((t_error){old_pwd, ERROR_MESG_NO_FILE, ENU_GENEREAL_FAILURE});
			}
		}
		else
	...
	if ((!dir && size == 1) || (dir && (!ft_strcmp(dir, "--") || !ft_strcmp(dir, ""))))
	{
		...
		{
			g_minishell.env->exit_status = ft_err_msg((t_error){home, ERROR_MESG_NO_FILE, ENU_GENEREAL_FAILURE});
			return ;
		}
	}
