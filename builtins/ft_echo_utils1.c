...
static int	handle_dash_n(char *normalized, char **result, int *i)
{
	int	start;
...
int	extract_word(char *normalized, char **result)
{
	int		i;
	...
	while (normalized[i])
	{
		if (normalized[i] == '-')
		{
			if (handle_dash_n(normalized, result, &i))
				return (1);
		}
		else
		{
...
char	*extract_content_after_flags(char *str)
{
	...
	if (!normalized)
		return (NULL);
	extract_word(normalized, &result);
	return (result);
}
